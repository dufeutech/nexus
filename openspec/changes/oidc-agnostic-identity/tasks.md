...
- [ ] 1.1 Define `AuthzResolver` in `identity-rs/core` — resolve a subject's effective
...
- [ ] 1.2 Define `AuthzAuthoring` in `core` — assign/revoke role, grant/revoke
      entitlement, suspend/reactivate. Domain-language, storage-agnostic.
- [ ] 1.3 Define the `AuthzFacts` value type (roles/entitlements/is_suspended) with the
      deny-by-default zero value.

## 2. Nexus-native adapter (Model 1: identity store)

- [ ] 2.1 Implement `AuthzAuthoring` over `PgProfileStore` as read-merge-write into
...
- [ ] 2.2 Implement `AuthzResolver` over `PgProfileStore` (`get(sub)` → `AuthzFacts`);
      absent profile → deny-by-default zero value.
- [ ] 2.3 Confirm the three writers (authz authoring + membership projection + any
      display materialization) converge via the version guard without clobbering.

## 3. Admin authoring surface + bootstrap

- [ ] 3.1 New identity-plane admin API (auth-gated like control-plane `CONTROL_AUTH_TOKEN`:
...
- [ ] 3.2 Bootstrap: grant an admin role to a configured bootstrap-admin subject at
      startup iff no administrator exists (idempotent, break-glass, documented).

## 4. Enrichment sourced from nexus (sidecar)

- [ ] 4.1 `sidecar extract_identity`: stop reading the token `roles` claim.
- [ ] 4.2 `enrich_response`: source `x-user-roles`, `x-user-entitlements`,
...
- [ ] 4.3 Leave the edge gate untouched (still compares injected → required); confirm
      the compare contract holds against nexus-sourced headers.

## 5. Delete the ZITADEL directory integration

- [ ] 5.1 Delete `identity-rs/reconciler/` and `identity-rs/sync-worker/` (binaries +
      workspace members + compose services + Helm templates).
- [ ] 5.2 Delete `core/reconcile.rs` and `core/sync.rs` and the ZITADEL wire-shape
      parsing, PAT handling, and Actions-webhook registration.
- [ ] 5.3 Remove `deploy/helm/identity-plane/templates/{reconciler,sync-worker}.yaml`,
...
- [ ] 5.4 `core/profile.rs`: shed `org_id` (IdP-only); keep `home_org` informational
      (`identity-workspace-authz`). Update the version-guard / `with_*` merges.

## 6. Provisioning migration

- [ ] 6.1 Re-author the current ZITADEL-sourced grants as nexus authorization for
...
- [ ] 7.1 `deploy/README.md` + `nexus-upstream-requirements.md`: the AuthN/AuthZ
...
- [ ] 8.1 A role-claiming token confers nothing (deny-by-default; spec R1).
- [ ] 8.2 Newly authenticated subject with no grants: auth-only route admits
      unprivileged, role/entitlement route 403 (spec R2).
- [ ] 8.3 Suspend → subsequent requests denied within seconds, no re-auth; grant role →
      route passes within seconds; revoke → stops passing (spec R3).
- [ ] 8.4 Bootstrap: from an empty store, the bootstrap admin can author the first
      grants (spec R4).
- [ ] 8.5 Full-lab boot with reconciler + sync-worker deleted: edge gate green,
