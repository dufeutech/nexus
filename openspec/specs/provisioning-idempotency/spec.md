...
Safe-to-retry resource creation on the admin provisioning surface: an optional
caller-supplied idempotency key makes creation replayable (the replay returns the
originally created resource), and create vs. reconfigure are distinct, non-overlapping
operations. This replaces the idempotence that previously rode on caller-supplied ids
being primary keys (retired by server-minted ids).
