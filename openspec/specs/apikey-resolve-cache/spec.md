# apikey-resolve-cache

## Purpose

Relieve the api-key resolve step on the identity sidecar hot path — the one per-request DB
hit that does not collapse on repeats — with an **optional, fail-closed** cache of api-key
resolutions. An app on a single key drives sustained high-RPS traffic against the same row;
this capability lets an operator opt in to caching the small **working set** of keys
*actively presenting traffic* (never the unbounded keyspace), so resolve cost scales with
active traffic rather than request volume. It owns only the observable contract of that
cache: opt-in and default-off; positive-only, expiry-respecting entries; bounded capacity
with working-set eviction; and preservation of the live-revocation guarantee via
change-signal-driven targeted invalidation with a self-healing fallback. It adds a behavior
layer *beneath* `customer-api-keys` (whose "revocation within seconds" bar it honors but
does not alter) and never changes the hash/pepper construction, key issuance, or the
