# box-telemetry-contract

## Purpose

The telemetry contract between nexus and any box on the internal network — the
observability twin of the trusted-header contract. nexus provides exactly one
collection endpoint accepting every signal (traces, metrics, logs) over the
openly standardized telemetry protocol, plus edge-rooted trace context and a
fail-open guarantee; a compliant box emits continued traces, RED metrics as
aggregatable distributions, trace-correlated structured logs, and standard
resource identity, under the same PII hygiene as the edge access log. The
mechanism is an OpenTelemetry Collector in front of per-signal stores queried
in one shared visualization stack; this spec is tool-agnostic and states only
