...
Decide authorization by applying declarative **policies-as-data** to a request's
principal, action, resource, and context — the platform's L2 authorization layer.
Decisions are **deny-by-default** and **fail-closed** (a missing or unparseable input
denies, never assumes a permissive default), each carries an **auditable reason**, and a
policy change takes effect **as data** without a code change. The decision is
**decoupled** from the enforcement surface that requests it, so enforcement points stay
thin adapters that translate a request into a decision query and act on the result (the
edge gate is the first consumer).
