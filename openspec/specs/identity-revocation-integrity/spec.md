...
Protects the integrity and freshness of the revocation-sensitive signals
(`x-user-entitlements`, `x-user-suspended`) that decide whether a user is cut off
right now. These signals were deliberately left out of the long-lived signed identity
contract to preserve liveness, but that left them riding bare, unsigned trust. This
capability closes that gap: the signals must be authenticated as nexus-authored and
unforgeable, without weakening the bounded freshness that is the whole reason they
live outside the signed contract.
