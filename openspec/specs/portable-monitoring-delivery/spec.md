# portable-monitoring-delivery

## Purpose

The portability contract for nexus's monitoring artifacts: the alerting/recording
rules and dashboards that realize the service SLO policy are generated from a single
source, carry query content portable across compatible metric backends, and can be
delivered both to backends that provide a rules/monitor controller and to backends that
provide none. Choosing a backend or a delivery form is configuration that changes no
telemetry producer and no query content. This keeps the monitoring surface working
everywhere nexus runs — from an operator-based cluster to a lean operator-less one to a
clean local checkout — without coupling nexus to any one backend or delivery mechanism.
This spec states only observable behavior; it is language-, framework-, and
vendor-agnostic. It complements `service-slo-policy` (which owns the objective and
burn-rate semantics) and `box-telemetry-contract` (which owns store-agnostic exposition).

## Requirements

### Requirement: Monitoring artifacts derive from a single source

The alerting/recording rules and dashboards that realize the SLO policy SHALL be
generated from a single declarative source. Every supported delivery form SHALL be
a rendering of that source; no delivery form SHALL carry hand-maintained rule or
query content that can diverge from the source.
