...
The canonical contract for resolving an inbound hostname to a workspace (tenant): how a
request's host is canonicalized and matched against registered domain rows — exact match first,
then a single-label wildcard, else fail closed with no tenant. It fixes the coexistence of an
apex and its wildcard, most-specific-wins precedence, single-label-only wildcard semantics, and
the requirement that request routing and on-demand certificate authorization resolve the identical
host set through one shared matcher. This spec is language-agnostic and states only the observable
behavior.
