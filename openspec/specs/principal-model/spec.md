...
Normalize every verified credential into a single **principal** shape — a kind (human user, api key, or
service), a subject, and an optional on-behalf-of subject — so authorization operates on the principal
alone, independent of which mechanism authenticated it. Kind, subject, and on-behalf-of are
system-authored from the verified credential and never caller-asserted. A verified credential that
resolves to no authority is rejected and receives no signed assertion. Principal kind (what
authenticated) stays orthogonal to workspace member type (a role held within a workspace).
