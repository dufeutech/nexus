# routing-invalidation-propagation

## Purpose

Keep every router instance's cache converged to the routing source-of-truth, including across a
region edge: when a routing record changes, the affected key is evicted on every instance —
regardless of whether it shares the writer's local signaling scope — so the next request
re-resolves current state. Delivery is best-effort with a bounded-staleness TTL backstop as the
correctness floor, and the resulting eviction is identical regardless of which transport carried
the signal.
