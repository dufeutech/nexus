# certificate-store-durability

## Purpose

The contract for the fleet-shared, durable certificate store behind on-demand TLS: any edge node
serves any customer domain from one shared store, concurrent first-time demand is single-flighted to
at most one issuance, stored certificates survive the loss of any single node, and per-node memory is
bounded to the active working set rather than the total domain population. This spec is
language-agnostic and states only the observable behavior.
