...
The cost guarantees of the telemetry stack — the economic twin of the box
telemetry contract. Each signal (traces, metrics, logs) is retained on a
storage tier that scales independently of any single host's disk, under an
explicit, operator-owned retention bound sized to that signal's purpose; and a
fail-safe cost ceiling at the single collection egress bounds what any one
producer can cost in metric-series cardinality and log/telemetry volume, so a
misbehaving or hostile producer degrades ITS OWN telemetry fidelity rather than
the shared bill, the store's health, or any request path. Trace cost stays
governed by the edge's head-sampling decision alone — no downstream tail
buffering. The mechanism is standard configuration of the stores and the
collection layer already in force (object storage + retention policy, egress
cardinality/volume guards); this spec is tool-agnostic and states only the
observable behavior.
