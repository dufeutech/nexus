...
The observable contract for validating the edge's sustained **capacity** — throughput and
tail latency under load — as distinct from correctness. The correctness gate proves the edge
enforces its contract; this capability proves how much traffic it sustains and at what tail
latency, and gates that against operator-set SLOs. This spec is language- and tool-agnostic
and states only observable behavior.
