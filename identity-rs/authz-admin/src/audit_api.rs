//! Audit ledger read surface + named-admin-token provisioning + retention for
//! authz-admin (admin-action-audit D4–D7) — the identity plane's twin of the
//! control plane's `audit_api`.
//!
...
use identity_core::audit::{AuditCtx, AuditQuery, InvalidQueryBound};
use identity_core::store::BoxError;
use store_postgres::{PgAdminAuditStore, PgAuditMaintenance};

use crate::{internal, App, METRICS};
...
pub(crate) async fn list_audit_events(
    ...
    match s.audit.query_audit_events(&query).await {
        Ok(events) => {
...
pub(crate) async fn export_audit_events(
    ...
    if let Err(e) = s.audit.query_audit_events(&probe).await {
    ...
    let body_stream = stream::unfold(init, |mut st| async move {
        ...
        match st.audit.query_audit_events(&query).await {
...
mod tests {
    ...
    fn retention_defaults_and_floors() {
        ...
        // Below the floor / garbage → refused, never a silent default.
        assert!(retention_days_from_env("364").is_err(), "below-floor retention must refuse");
        assert!(retention_days_from_env("nope").is_err(), "unparseable must refuse");
    }
}
