...
#      backend-origin NetworkPolicy, and the zitadel_jwks cluster carries the
...
IP_BASE=(--set postgres.url=postgres://u:p@h:5432/identity
         --set zitadel.patSecret.value=dummy
...
IP_JWKS_TLS=(--set zitadel.jwksTls.enabled=true)
IP_JWKS_TRUST=(--set zitadel.jwksPlaintextTrustedPath=true)
...
ok "$(printf '%s' "$OUT" | grep -q 'UpstreamTlsContext' && echo 1 || echo 0)" "zitadel_jwks cluster carries UpstreamTlsContext"
...
EP_BASE=(--set identity-plane.postgres.url=postgres://u:p@h:5432/identity
         --set identity-plane.zitadel.patSecret.value=dummy
...
expect_fail "umbrella: no origin-enforcement choice refuses to render" "origin-enforcement" \
  edge-platform "${EP_BASE[@]}" --set identity-plane.zitadel.jwksTls.enabled=true
expect_fail "umbrella: no JWKS transport-integrity choice refuses to render" "trust anchor" \
  edge-platform "${EP_BASE[@]}" "${EP_ORIGIN_NP[@]}"
OUT=$(render edge-platform "${EP_BASE[@]}" "${EP_ORIGIN_NP[@]}" --set identity-plane.zitadel.jwksTls.enabled=true) ; rc=$?
...
ok "$(printf '%s' "$OUT" | grep -q 'provider_name: zitadel' && echo 1 || echo 0)" "rendered edge keeps the catch-all provider rule (absent signal fails closed)"
for CFG in edge/envoy.yaml deploy/compose/envoy/envoy.yaml; do
  ...
  ok "$(grep -q 'provider_name: zitadel' "$CFG" && echo 1 || echo 0)" "$CFG keeps the catch-all provider rule"
done
