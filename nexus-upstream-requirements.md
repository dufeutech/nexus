...
| `x-identity-contract` (signed ES256 JWS)           | the acting-identity contract, now a **signed token** (`identity-contract-signing`): carries `iss`/`aud`/`exp` + the acting identity, with the contract version as the `ctr` claim. Minted **only** for an authenticated member (non-member/anonymous/miss carry none). Boxes fetch the JWKS at `<identity-plane>:9210/.well-known/jwks.json`, verify signature + `iss` + `aud` (= the box's `x-route-pool`) + `exp`, and reject an absent/unverifiable token or unknown `ctr` on enriched routes. There is no plain-string form. Origin trust stays the primary anti-bypass control (augment, not replace). | shipped (backend verifies the token; `iss`=`https://identity.nexus`, `aud`=the box's route-pool name) |
