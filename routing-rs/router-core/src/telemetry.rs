//! telemetry — the ONE place the routing plane assembles observability.
//!
//! Every routing-plane binary (tenant-router, control-plane) calls [`init`] once
//! at startup and holds the returned [`TelemetryGuard`] for the process lifetime.
//! This is the box-telemetry-contract made first-party: one endpoint, all signals,
//! standard resource identity, trace-correlated logs, fail-open.
//!
...
//!   so a pre-1.0 version bump is a two-file change (this + the identity-plane twin
//!   `identity_core::telemetry`, which is kept byte-for-byte identical below the
    ...
    if let Some(active) = providers.as_ref() {
        layers.push(
            tracing_opentelemetry::layer()
                .with_tracer(active.tracer.tracer("router-core"))
                .boxed(),
        );
        layers.push(
